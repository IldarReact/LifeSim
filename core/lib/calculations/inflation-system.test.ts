...
describe('Inflation System', () => {
  ...
  describe('applyYearlyInflation', () => {
    it('should add new inflation to history', () => {
      ...
      // История должна содержать старую и новую инфляцию
      expect(result.newEconomy.inflationHistory).toHaveLength(2)
      expect(result.newEconomy.inflationHistory![0]).toBe(3) // Базовая
      expect(result.newEconomy.inflationHistory![1]).toBeGreaterThan(0) // Новая
    })
