...
export const createBusinessSlice: GameStateCreator<BusinessSlice> = (set, get) => ({
  openBusiness: (business, upfrontCost) => {
    const s = get()
    if (typeof s.openBusiness === 'function') return s.openBusiness(business, upfrontCost)
  ...
  hireEmployee: (businessId, candidate) => {
    const s = get()
    if (typeof s.hireEmployee === 'function') return s.hireEmployee(businessId, candidate)
  ...
  fireEmployee: (businessId, employeeId) => {
    const s = get()
    if (typeof s.fireEmployee === 'function') return s.fireEmployee(businessId, employeeId)
    console.warn('[business-slice] fireEmployee delegated, but target not found')
  },

  closeBusiness: (businessId) => {
    const s = get()
    if (typeof s.closeBusiness === 'function') return s.closeBusiness(businessId)
  ...
  unfreezeBusiness: (businessId) => {
    const s = get()
    if (typeof s.unfreezeBusiness === 'function') return s.unfreezeBusiness(businessId)
  ...
  changePrice: (businessId, newPrice) => {
    const s = get()
    if (typeof s.changePrice === 'function') return s.changePrice(businessId, newPrice)
  ...
  setQuantity: (businessId, newQuantity) => {
    const s = get()
    if (typeof s.setQuantity === 'function') return s.setQuantity(businessId, newQuantity)
  ...
  setAutoPurchase: (businessId, amount) => {
    const s = get()
    if (typeof s.setAutoPurchase === 'function') return s.setAutoPurchase(businessId, amount)
    console.warn('[business-slice] setAutoPurchase delegated, but target not found')
  },

  openBranch: (sourceBusinessId) => {
    const s = get()
    if (typeof s.openBranch === 'function') return s.openBranch(sourceBusinessId)
    console.warn('[business-slice] openBranch delegated, but target not found')
  },

  proposeAction: (businessId, type, payload) => {
    const s = get()
    if (typeof s.proposeAction === 'function') return s.proposeAction(businessId, type, payload)
  ...
  setPlayerManagerialRoles: (businessId, roles) => {
    ...
    if (typeof s.setPlayerManagerialRoles === 'function')
      return s.setPlayerManagerialRoles(businessId, roles)
  ...
  setPlayerEmploymentSalary: (businessId, salary) => {
    ...
    if (typeof s.setPlayerEmploymentSalary === 'function')
      return s.setPlayerEmploymentSalary(businessId, salary)
    console.warn('[business-slice] setPlayerEmploymentSalary delegated, but target not found')
  },

  freezeBusiness: (businessId) => {
    const s = get()
    if (typeof s.freezeBusiness === 'function') return s.freezeBusiness(businessId)
  ...
  joinBusinessAsEmployee: (businessId, role, salary) => {
    ...
    if (typeof s.joinBusinessAsEmployee === 'function')
      return s.joinBusinessAsEmployee(businessId, role, salary)
  ...
  setPlayerEmploymentEffort: (businessId, effortPercent) => {
    ...
    if (typeof s.setPlayerEmploymentEffort === 'function')
      return s.setPlayerEmploymentEffort(businessId, effortPercent)
  },

  setEmployeeEffort: (businessId, employeeId, effortPercent) => {
    const s = get()
    if (typeof s.setEmployeeEffort === 'function')
      return s.setEmployeeEffort(businessId, employeeId, effortPercent)
  },
  ...
  addPartnerToBusiness: (businessId, partnerId, partnerName, share, investment) => {
    ...
    if (typeof s.addPartnerToBusiness === 'function')
      return s.addPartnerToBusiness(businessId, partnerId, partnerName, share, investment)
  ...
  addSharedBusiness: (business) => {
    const s = get()
    if (typeof s.addSharedBusiness === 'function') return s.addSharedBusiness(business)
  ...
  addEmployeeToBusiness: (businessId, employeeName, role, salary, playerId, extraData) => {
    ...
    if (typeof s.addEmployeeToBusiness === 'function')
      return s.addEmployeeToBusiness(businessId, employeeName, role, salary, playerId, extraData)
  ...
  updateEmployeeInBusiness: (businessId, employeeId, updates) => {
    ...
    if (typeof s.updateEmployeeInBusiness === 'function')
      return s.updateEmployeeInBusiness(businessId, employeeId, updates)
