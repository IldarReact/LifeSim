    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="relative z-10 container mx-auto p-6 max-w-5xl">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-zinc-500 text-lg">Вклады, кредиты и твоя финансовая реальность</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {[
            ...
            { label: "Чистый капитал", value: `$${(totalDeposits - totalDebt).toLocaleString()}` },
          ].map((stat, i) => (
            <Card
              key={i}
              className="bg-white/6 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 text-center hover:bg-white/8 transition-all duration-300"
            >
        ...
        <div className="mb-12">
          ...
          {deposits.length === 0 ? (
            <Card className="bg-white/6 backdrop-blur-2xl border border-white/10 border-dashed rounded-3xl p-20 text-center">
            ...
            <div className="space-y-5">
              {deposits.map(deposit => (
                <Card
                  key={deposit.id}
                  className="bg-white/6 backdrop-blur-2xl border border-white/10 rounded-3xl p-7 hover:bg-white/8 transition-all duration-300"
                >
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-xl font-bold text-zinc-100">{deposit.name || "Вклад"}</h3>
                      <p className="text-zinc-500 text-sm">Сумма на счету</p>
                    </div>
          ...
          <Button className="w-full mt-8 bg-white/8 hover:bg-white/12 text-zinc-200 border border-white/20 backdrop-blur-2xl rounded-3xl h-14 text-lg font-medium">
            Открыть вклад
          </Button>
        </div>
        ...
        <div>
            ...
            <div className="space-y-5">
              {debts.map(debt => (
                ...
                >
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-xl font-bold text-zinc-100">{debt.name || "Кредит"}</h3>
                      <p className="text-zinc-500 text-sm">Остаток долга</p>
                    </div>
          ...
          <Button
            className="w-full mt-8 bg-white/8 hover:bg-white/12 text-zinc-200 border border-white/20 backdrop-blur-2xl rounded-3xl h-14 text-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={!hasIncome}
          >
            {!hasIncome ? "Кредит недоступен — нет дохода" : "Взять кредит"}
          </Button>
        </div>
      </div>
    </div>
