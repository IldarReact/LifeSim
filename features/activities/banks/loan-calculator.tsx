...
export function LoanCalculator() {
  ...
  const quarters = parseInt(termMonths) / 3 || 0;
  ...
  const enoughMoney = player.stats.money >= loanAmount;
    ...
    <Card className="p-6 bg-white/5 backdrop-blur-sm border-white/10">
      ...
      <div className="space-y-4 mb-6">
        ...
        <div>
          ...
          <Select
            value={loanType}
            onValueChange={(value) => setLoanType(value as Debt["type"])}
          >
            <SelectTrigger className="bg-white/5 border-white/10 text-white">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="consumer_credit">
                {loanTypeNames.consumer_credit}
              </SelectItem>
              <SelectItem value="mortgage">
                {loanTypeNames.mortgage}
              </SelectItem>
              <SelectItem value="student_loan">
                {loanTypeNames.student_loan}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {/* Срок кредита */}
        <div>
          <Label className="text-white/80 mb-2">Срок кредита</Label>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="text-xs text-white/60">Процентная ставка</div>
              <div className="text-2xl font-bold text-white">
                {interestRate.toFixed(2)}%
              </div>
              <div className="text-xs text-white/40">годовых</div>
            </div>

            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="text-xs text-white/60">Срок</div>
              <div className="text-2xl font-bold text-white">{quarters}</div>
              <div className="text-xs text-white/40">кварталов</div>
            </div>
          </div>

          <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
            <div className="flex justify-between">
              <span className="text-white/80">Платеж за квартал</span>
