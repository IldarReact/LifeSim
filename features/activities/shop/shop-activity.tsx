  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]">Магазин и недвижимость</h2>
          <p className="text-zinc-300 drop-shadow-[0_1px_4px_rgba(0,0,0,0.9)]">Еда, транспорт, жильё — всё в одном месте</p>
        </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`flex items-center gap-2.5 px-5 py-3 rounded-xl font-medium transition-all whitespace-nowrap backdrop-blur-md ${selectedCategory === cat.id
              ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg shadow-purple-500/30 border border-purple-400/30'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {items.map(item => {
          ...
          return (
            <Card
              key={item.id}
              className={`relative overflow-hidden transition-all duration-300 border-2 backdrop-blur-xl ${isCurrentHousing
