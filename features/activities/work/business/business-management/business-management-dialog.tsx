          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <p className="text-sm font-semibold text-white/70 uppercase tracking-wider">Активные роли:</p>
                <div className="flex flex-wrap gap-2">
                  {business.playerRoles.managerialRoles.map(role => (
                    <Badge key={role} variant="outline" className="bg-orange-500/10 text-orange-400 border-orange-500/20 py-1 px-3">
                      {ROLE_ICONS[role]} <span className="ml-2">{ROLE_LABELS[role]}</span>
                    </Badge>
                  ))}
                  {business.playerRoles.operationalRole && (
                    <Badge variant="outline" className="bg-purple-500/10 text-purple-400 border-purple-500/20 py-1 px-3">
                      {ROLE_ICONS[business.playerRoles.operationalRole]} <span className="ml-2">{ROLE_LABELS[business.playerRoles.operationalRole]} (Опер.)</span>
                    </Badge>
                  )}
                  {business.playerRoles.managerialRoles.length === 0 && !business.playerRoles.operationalRole && (
                    <p className="text-sm text-white/40 italic">Вы не участвуете в работе этого филиала</p>
                  )}
                </div>
                <p className="text-xs text-white/40 mt-2">
                  * Управленческие роли (Директор, Бухгалтер) назначаются автоматически, если нет наемных сотрудников.
                </p>
              </div>

              <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                <p className="text-sm font-semibold text-white/70 uppercase tracking-wider mb-3">Влияние на ваши статы:</p>
                {(() => {
                  const effects = calculateBusinessFinancials(business, true).playerStatEffects || { energy: 0, sanity: 0 };
                      ...
                      {energy !== 0 && (
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-white/60 flex items-center gap-2">
                            <Activity className="w-4 h-4 text-blue-400" /> Энергия
                          </span>
                          <span className="font-bold text-red-400">{energy > 0 ? '+' : ''}{energy} / кв</span>
                        </div>
                      )}
                      {sanity !== 0 && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-bold text-red-400">{sanity > 0 ? '+' : ''}{sanity} / кв</span>
                        </div>
                      )}
                    </div>
                  ) : (
                    <p className="text-sm text-white/40 italic">Нет влияния на статы</p>
                  );
                })()}
              </div>
            </div>
