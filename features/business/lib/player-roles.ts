import type { Business, Employee, EmployeeRole } from '@/core/types/business.types';
import type { Skill } from '@/core/types';
import type { StatEffect } from '@/core/types/stats.types';
import { EMPLOYEE_ROLES_CONFIG, getRoleConfig, isManagerialRole, isOperationalRole } from '../config/employee-roles.config';
...
export function calculatePlayerRoleEffects(business: Business): StatEffect {
  ...
  console.log(`[Business ${business.name}] Player role effects:`, totalEffect);
...
export function getPlayerRoleBusinessImpact(
  ...
  console.log(`[Business ${business.name}] Player impact:`, impact);
...
export function checkMinimumStaffing(business: Business): {
  ...
  // ✅ ИСПРАВЛЕНИЕ: Подсчитать только работников (worker)
  ...
  console.log(`[Business ${business.name}] Staffing check: Workers=${workerCount}/${minEmployees}, Total=${totalEmployees}, MissingRoles=${missingRoles.join(', ') || 'none'}`);
...
export function updateAutoAssignedRoles(business: Business): Business {
  ...
  const allRoles = Array.from(new Set([...currentRoles, ...autoRoles]));
